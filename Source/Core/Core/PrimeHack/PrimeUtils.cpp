...
enum class AspectMode {
  AM_4_3, AM_16_9, AM_16_10, AM_STRETCH,
};

static AspectMode get_aspect_mode() {
  int aspect_mode = Config::Get(Config::GFX_ASPECT_RATIO);
  switch (aspect_mode) {
  case 2:
  case 4:
    return AspectMode::AM_4_3;
  case 6:
    return AspectMode::AM_16_10;
  case 3:
    if (Config::Get(Config::GFX_WIDESCREEN_HACK)) {
      return AspectMode::AM_STRETCH;
    }
  [[fallthrough]];
  default:
  case 0:
...
void handle_cursor(u32 x_address, u32 y_address, Region region) {
  ...
  if (aspect_mode == AspectMode::AM_STRETCH) {
    const float cur_width = g_renderer->GetBackbufferWidth();
    const float cur_height = g_renderer->GetBackbufferHeight();
    const float render_aspect = cur_width / cur_height;
    handle_wiimote_IR(x_address, y_address, region, (region == Region::PAL ? kPalHalfHRange : kNtscHalfHRange), kMenuHalfVRange, AspectMode::AM_16_9, render_aspect);
  } else {
...
void handle_reticle(u32 x_address, u32 y_address, Region region, float fov) {
  ...
  switch (aspect_mode) {
  default:
  case AspectMode::AM_16_9:
  case AspectMode::AM_STRETCH:
    base_cursor_range_h = (region == Region::PAL ? kPalHalfVRange_16_9 : kNtscHalfVRange_16_9) * fov_scaling;
    break;
  case AspectMode::AM_16_10:
    base_cursor_range_h = (region == Region::PAL ? kPalHalfVRange_16_10 : kNtscHalfVRange_16_10) * fov_scaling;
    break;
  case AspectMode::AM_4_3:
